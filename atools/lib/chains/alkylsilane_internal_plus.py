...
class Alkylsilane(mb.Compound):
    ...
    def __init__(self, chain_length, internal_group, locations, terminal_group):
        ...
        name = 'o'
        module = __import__('atools.lib.chains.compound_with_ports.'+name)
        O = getattr(module.lib.chains.compound_with_ports, name.title())

        name = 'ch2'
        module = __import__('atools.lib.chains.compound_with_ports.'+name)
        CH2 = getattr(module.lib.chains.compound_with_ports, name.title())
        
        if internal_group=='peg':
            self.add(tgroup, 'terminal_group')
        
            chain = mb.recipes.Polymer(monomers=[CH2(), O()], sequence='AAB',
                                    n=math.floor(chain_length/3), port_labels=('up', 'down'))
