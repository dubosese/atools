...
def unwrap_xtc(xtc_file, coordinate_file):
    out_file = xtc_file[-1] + '-uw.xtc
    os.system('gmx trjconv -f {} -o {} -s {} -pbc nojump'.format(xtc_file,
        out_file, coordinate_file))

def write_lammps_data(filename, xyz, box,types=None, charges=None, mol_id=None,
                      mass_dict=None, velocities=None, image_flags=None, bonds=None,
                      bond_types=None, angles=None, angle_types=None, dihedrals=None,
                      dihedral_types=None):
    ...
    for i,coords in enumerate(xyz):
        out.write('{:d}\t{:.0f}\t{:d}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.6f}\t{:.0f}'
                  '\t{:.0f}\t{:.0f}\n'.format(i + 1, mol_id[i], types[i],
                                              charges[i], coords[0], coords[1], 
                                              coords[2], image_flags[i, 0],
                                              image_flags[i, 1], image_flags[i, 2]))
    ...
    if bonds is not None:
        ...
        for i,bond in enumerate(bonds):
            out.write('{:d}\t{:.0f}\t{:d}\t{:d}\n'.format(i + 1, bond_types[i],
                                                          bond[0], bond[1]))
    if angles is not None:
        out.write('\nAngles\n\n')
        for i,angle in enumerate(angles):
            out.write('{:d}\t{:.0f}\t{:d}\t{:d}\t{:d}\n'.format(i + 1,
                angle_types[i], angle[0], angle[1],angle[2]))
    if dihedrals is not None:
        out.write('\nDihedrals\n\n')
        for i,dihedral in enumerate(dihedrals):
            out.write('{:d}\t{:.0f}\t{:d}\t{:d}\t{:d}\t{:d}\n'.format(i + 1,
                dihedral_types[i], dihedral[0], dihedral[1], dihedral[2],
                dihedral[3]))
